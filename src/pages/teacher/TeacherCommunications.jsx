...
const COMM_BASE = '/portal/docente/comunicados';

export function TeacherCommunications() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user || communications.length === 0) return;
    const run = async () => {
      ...
      setReadStatus(Object.fromEntries(statuses));
    };
    run();
  }, [communications, user]);

  const openCommunication = (comm) => navigate(`${COMM_BASE}/${comm.id}`);

  const handleOpenFirstUnread = () => {
    if (unreadRequired.length > 0) navigate(`${COMM_BASE}/${unreadRequired[0].id}`);
  };
    ...
    <div className="container page-container communications-page">
      <div className="dashboard-header dashboard-header--compact communications-header">
        <div>
          ...
          <p className="dashboard-subtitle">Mensajes y avisos de la institución.</p>
        </div>
      ...
      {hasUnreadRequired && (
        <div className="alert alert--warning" style={{ marginBottom: 'var(--spacing-md)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--spacing-md)' }}>
            ...
            <button className="btn btn--primary btn--sm" onClick={handleOpenFirstUnread} style={{ flexShrink: 0 }}>
              Leer ahora
            </button>
          </div>
        </div>
        ...
        <div className="card__body">
          <div className="communications-filters__row">
            <div className="communications-search">
              <label className="form-label" htmlFor="tc-search">Buscar</label>
            ...
            <div className="communications-select">
              <label className="form-label" htmlFor="tc-sender">Remitente</label>
              <select id="tc-sender" value={senderFilter} onChange={(e) => setSenderFilter(e.target.value)} className="form-input">
                <option value="all">Todos los remitentes</option>
                {senders.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div className="communications-select">
              <label className="form-label" htmlFor="tc-date">Fecha</label>
              <select id="tc-date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="form-input">
            ...
            <div className="communications-filters__actions">
              <button className="btn btn--outline btn--sm" onClick={() => { setSenderFilter('all'); setDateFilter('all'); setSearchTerm(''); }}>
                Limpiar
              </button>
            </div>
          </div>
