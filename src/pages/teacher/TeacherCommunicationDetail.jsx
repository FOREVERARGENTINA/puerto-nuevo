...
const COMM_LIST = '/portal/docente/comunicados';

export function TeacherCommunicationDetail() {
  const { id } = useParams();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    readReceiptsService.hasUserRead(communication.id, user.uid).then((result) => {
      if (result.success) setHasRead(result.hasRead);
    });
  }, [communication, user]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    readReceiptsService.markAsRead(communication.id, user.uid, user.displayName || user.email).then((result) => {
      if (!cancelled && result.success) setHasRead(true);
    });
  ...
  useEffect(() => {
    ...
    usersService.getUserById(communication.sentBy)
      .then((res) => { if (res.success) setSenderName(res.user.displayName || res.user.email || '—'); })
      .catch((err) => console.error('Error cargando remitente:', err))
  ...
  const handleMarkAsRead = async () => {
    ...
    setConfirming(true);
    try {
      const result = await readReceiptsService.markAsRead(communication.id, user.uid, user.displayName || user.email);
      if (result.success) setHasRead(true);
    } catch (err) {
  ...
  if (error || !communication) {
    return (
      <div className="container page-container">
        <div className="alert alert--error">{error || 'Comunicado no encontrado'}</div>
        <Link to={COMM_LIST} className="btn btn--outline btn--back" style={{ marginTop: 'var(--spacing-md)' }}>
    ...
    <div className="container page-container">
      <div className="dashboard-header dashboard-header--compact">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
          {communication.requiereLecturaObligatoria && (
            hasRead ? (
              <span className="badge badge--success"><Icon name="check" size={14} /> Leído</span>
            ) : (
              <span className="badge badge--warning"><Icon name="alert" size={14} /> Requiere lectura</span>
            )
          )}
          <Link to={COMM_LIST} className="btn btn--outline btn--back">
            <Icon name="chevron-left" size={16} />
            Volver
          </Link>
        </div>
      ...
      <div className="card">
        <div className="card__body">
          <CommunicationRichContent body={communication.body} bodyRich={communication.bodyRich} />
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {communication.attachments.map((attachment, i) => (
                  <li key={i} style={{ marginBottom: 'var(--spacing-xs)' }}>
                    <a href={attachment.url} target="_blank" rel="noopener noreferrer" className="btn btn--text" style={{ padding: 'var(--spacing-xs) 0' }} title={attachment.name || `Archivo adjunto ${i + 1}`}>
      ...
      {communication.requiereLecturaObligatoria && !hasRead && (
        <div className="card" style={{ marginTop: 'var(--spacing-md)', textAlign: 'center' }}>
          <div className="card__body">
            ...
            <button className="btn btn--primary btn--lg" onClick={handleMarkAsRead} disabled={confirming} style={{ minWidth: '200px' }}>
