...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Custom force to pull students to their classroom centers
    ...
    // Adjust link force: stronger when docente is active
    const isDocenteActive = selectedRoles.includes('docente');
    fg.d3Force('link')?.strength((link: any) => {
      const source = link.source as any;
      const target = link.target as any;
      
      // If they are in different classrooms, weaken the link significantly
      ...
      if (link.type === 'padre-hijo') return 0.2;
      return 0.05;
    });
